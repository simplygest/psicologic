...
if (function_exists('app_auto_schema_migrations_enabled') && app_auto_schema_migrations_enabled()) {
    ...
    if ($settings_table && $settings_table->num_rows > 0) {
        ...
        $can_read_delivery_mode = $delivery_column && $delivery_column->num_rows > 0;
    }
}
if ($can_read_delivery_mode) {
    $tenant_id = current_tenant_id();
    $delivery_res = $mysqli->query("SELECT appointment_delivery_mode FROM payment_settings WHERE tenant_id = $tenant_id");
    if ($delivery_res && $delivery_row = $delivery_res->fetch_assoc()) {
